...
def pss(n_samples=61,N=0,amp=.5,T=PERIOD,puls_width=N_SAMPLES/2):
    ...
    return np.array(pss_seq,dtype=np.complex64)

def cos_squared(n_samples=61,N=0,amp=.5,T=PERIOD,puls_width=N_SAMPLES/2):
    t = np.arange(0,n_samples,dtype=np.complex64)
    freq = 1/T
    return amp*np.cos(2*np.pi*freq*t) **2

def rect(n_samples=61,N=0,amp=.5,T=PERIOD,puls_width=N_SAMPLES/2):
    rect = np.arange(n_samples) % T < puls_width
    return np.array([float(sample) for sample in rect],dtype=np.complex64)
